...
const setCurrentMusic = () => {
  if (typeof musicPlayer.value === 'undefined') return

  const track = playList.value[currentTrackNumber.value]
  if (!track) return
  if (!musicPlayer.value.paused) {
    musicPlayer.value.pause()
    musicPlayer.value.currentTime = 0
  }
  musicPlayer.value.src = track.fileSrc
  coverImageRef.value = track.coverSrc
  musicTitle.value = track.title
  currentCopyText.value = track.copyText
}
const playMusic = () => {
