...
interface BlogContent {
  ...
  date: string
  path: string
  body: unknown // include if you render the body with <ContentRenderer>
}
...
export const useBlogStore = defineStore('blog', () => {
  ...
  const currentContent = ref<BlogContent | null>(null)
  ...
  const fetchBlogContent = async () => {
    ...
    const res = await queryCollection('blog')
  ...
  const onPrevClick = () => {
    ...
    if (
      ...
      currentPath.value = newTargetDirectory ?? currentPath.value
    }
    if (historyRef.value.length === currentPathIdx.value) {
      isRootDir.value = true
    }
